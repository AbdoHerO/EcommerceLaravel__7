                        ...
                        <div id="datatable_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap no-footer">
                            <div class="row">
                                ...
                                <div class="col-sm-6">
                                    ...
                                    <div><a href="{{URL('dashboard/productImage/create ')}}"
                                    ...
                                    <table id="datatable" class="table table-striped table-bordered dataTable no-footer"
                                        ...
                                        <thead>
                                            <tr role="row">
                                                ...
                                                <th class="sorting" tabindex="0" aria-controls="datatable" rowspan="1"
                                                    colspan="1" style="width: 44px;">Image Name</th>
                                                <th class="sorting" tabindex="0" aria-controls="datatable" rowspan="1"
                                        ...
                                        <tbody>
                                            {{ asset('backendAdmin/vendors/gauge.js/dist/gauge.min.js') }}
                                            @foreach ($productImage as $prodimg)
                                            <tr role="row" class="odd">
                                                <td class="sorting_1">{{ $prodimg->id }}</td>
                                                <td><img src="{{ asset('images/'.$prodimg->image_product) }}" style="width: 50px;height: 50px;" alt=""></td>
                                                <td>{{ $prodimg->product->product_name }}</td>
                                                <td>{{ $prodimg->image_title }}</td>
                                                <td>{{ $prodimg->status }}</td>
                                                <td>
                                                    <div class="col-md-6">
                                                        <div class="btn-group">
                                                            <form
                                                                action="{{URL('dashboard/productImage/' . $prodimg->id . '/edit')}}"
                                                                method="get">
                                                                @csrf
                                                                <button type="submit"
                                                                    class="btn btn-secondary ml-1 mr-1">Edit</button>
                                                            </form>
                                                            {{--action="{{ route('product.destroy', $prodimg->id) }}" --}}
                                                            <form  method="post" action="{{URL('dashboard/productImage/' . $prodimg->id)}}">
                                                            ...
                                                            <form
                                                                action="{{URL('dashboard/productImage/' . $prodimg->id)}}"
                                                                method="get">
